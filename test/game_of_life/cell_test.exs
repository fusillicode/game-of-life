defmodule GameOfLife.CellTest do
  ...
  test "coords returns [0,0] if the initial coords are [0,0]", state do
    assert GameOfLife.Cell.coords(state[:pid]) == [0,0]
  end

  test "coords returns true if the initial state is true" do
    {:ok, pid} = GameOfLife.Cell.new true, [1,0]
    assert GameOfLife.Cell.coords(pid) == [1,0]
  end
  ...
  test "x returns 1 if the initial x coord is 1" do
    {:ok, pid} = GameOfLife.Cell.new true, [1,0]
  ...
  test "y returns 1 if the initial y coord is 1" do
